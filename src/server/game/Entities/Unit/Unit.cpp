...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    SpellInfo const* spellProto = sSpellStore.LookupEntry(damageInfo->SpellID);
    if (spellProto == NULL)
    {
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            // item neutral spell
            if ((*itr)->GetSpellInfo()->GetEquippedItemClass() == -1)
            {
        ...
        if (Aura* foundAura = GetOwnedAura(newAura->Id, casterGUID, (sSpellMgr->GetSpellCustomAttr(newAura->Id) & SPELL_ATTR0_CU_ENCHANT_PROC) ? castItemGUID : 0, 0))
        {
            ...
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                ...
                if (baseAmount)
                    ...
                    bp = foundAura->GetSpellInfo()->CalculateSimpleValue(i);
...
void Unit::_AddAura(UnitAura* aura, Unit* caster)
{
    ...
    if (aura->IsSingleTarget())
    {
        ...
        {
            if ((*itr) != aura &&
                IsSingleTargetSpells((*itr)->GetSpellInfo(), aura->GetSpellInfo()))
            {
                (*itr)->Remove();
                itr = scAuras.begin();
            }
...
void Unit::_ApplyAura(AuraApplication * aurApp, uint8 effMask)
{
    ...
    if (aura->GetSpellInfo()->GetAuraInterruptFlags() & AURA_INTERRUPT_FLAG_NOT_SEATED && !IsSitState())
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    if (aura->GetSpellInfo()->GetAuraInterruptFlags())
    {
...
void Unit::RemoveAurasDueToSpellByDispel(uint32 spellId, uint64 casterGUID, Unit* dispeller, uint8 chargesRemoved/*= 1*/)
{
    for (AuraMap::iterator iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            switch (aura->GetSpellInfo()->GetSpellFamilyName())
            {
                case SPELLFAMILY_WARLOCK:
                {
                    // Unstable Affliction (crash if before removeaura?)                    
                    if (aura->GetSpellInfo()->GetSpellClassOptions()->SpellFamilyFlags[1] & 0x0100)
                    {
                        Unit* caster = aura->GetCaster();
                        if (!caster)
                            break;
                ...
                {
                    //Lifebloom
                    if (aura->GetSpellInfo()->GetSpellClassOptions()->SpellFamilyFlags[1] & 0x10)
                    {
                        if (AuraEffect const* aurEff = aura->GetEffect(EFFECT_1))
                        {
                            ...
                            {
                                int32 mana = CalculatePctU(caster->GetCreateMana(), aura->GetSpellInfo()->GetManaCostPercentage()) * chargesRemoved / 2;
                ...
                {
                    // Flame Shock
                    if (aura->GetSpellInfo()->GetSpellClassOptions()->SpellFamilyFlags[0] & 0x10000000)
                    {
                        if (Unit* caster = aura->GetCaster())
                        {
...
void Unit::RemoveAurasWithInterruptFlags(uint32 flag, uint32 except)
{
    ...
    for (AuraApplicationList::iterator iter = m_interruptableAuras.begin(); iter != m_interruptableAuras.end();)
    {
        Aura* aura = (*iter)->GetBase();
        ++iter;
        if ((aura->GetSpellInfo()->GetAuraInterruptFlags() & flag) && (!except || aura->GetId() != except))
        {
            uint32 removedAuras = m_removedAurasCount;
            RemoveAura(aura);
            if (m_removedAurasCount > removedAuras + 1)
...
void Unit::RemoveAllAurasExceptType(AuraType type)
{
    ...
    {
        Aura const* aura = iter->second->GetBase();
        if (!IsSpellHaveAura(aura->GetSpellInfo(), type))
            _UnapplyAura(iter, AURA_REMOVE_BY_DEFAULT);
        else
            ++iter;
    }
...
bool Unit::HasNegativeAuraWithInterruptFlag(uint32 flag, uint64 guid)
{
    ...
    {
        if (!(*iter)->IsPositive() && (*iter)->GetBase()->GetSpellInfo()->GetAuraInterruptFlags() & flag && (!guid || (*iter)->GetBase()->GetCasterGUID() == guid))
            return true;
    }
    return false;
}
...
uint32 Unit::GetDiseasesByCaster(uint64 casterGUID, bool remove)
{
    ...
    for (AuraType const* itr = &diseaseAuraTypes[0]; itr && itr[0] != SPELL_AURA_NONE; ++itr)
    {
        for (AuraEffectList::iterator i = m_modAuras[*itr].begin(); i != m_modAuras[*itr].end();)
        {
            ...
            if ((*i)->GetSpellInfo()->GetDispel() == DISPEL_DISEASE
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    {
        SpellInfo const* createBySpell = sSpellStore.LookupEntry(gameObj->GetSpellId());
...
void Unit::RemoveGameObject(GameObject* gameObj, bool del)
{
    ...
    if (uint32 spellid = gameObj->GetSpellId())
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            SpellInfo const* createBySpell = sSpellStore.LookupEntry(spellid);
...
bool Unit::HandleHasteAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    SpellInfo const* triggerEntry = sSpellStore.LookupEntry(triggered_spell_id);
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            switch(dummySpell->Id)
            {
                ...
                case 56372:
                {
                    ...
                    {
                        SpellInfo const* cdSpell = sSpellStore.LookupEntry(itr->first);
                        // Frost Nova
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 55680:
                {
                    ...
                    SpellInfo const* GoPoH = sSpellStore.LookupEntry(triggered_spell_id);
                    if (!GoPoH)
                        return false;
                ...
                case 70770:
                    ...
                    if (procSpell->GetSpellClassOptions()->SpellFamilyFlags[0] & 0x800)
                    {
                        ...
                        SpellInfo const* blessHealing = sSpellStore.LookupEntry(triggered_spell_id);
                        if (!blessHealing)
                            return false;
            ...
            switch(dummySpell->Id)
            {
                ...
                case 70723:
                {
                    ...
                    if ((procSpell->GetSpellClassOptions()->SpellFamilyFlags[0] & 0x5) && (procEx & PROC_EX_CRITICAL_HIT))
                    {
                        ...
                        SpellInfo const* triggeredSpell = sSpellStore.LookupEntry(triggered_spell_id);
                        if (!triggeredSpell)
                            return false;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    SpellInfo const* windfurySpellEntry = sSpellStore.LookupEntry(spellId);
            ...
            if (dummySpell->SpellIconID == 2287)
            {
                ...
                {
                    uint32 spell = aurEff->GetSpellInfo()->GetEffectTriggerSpell(aurEff->GetEffIndex());
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->SpellIconID == 1939 && GetTypeId() == TYPEID_PLAYER)
            {
                ...
                for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
                {
                    ...
                    SpellInfo const* spellProto = sSpellStore.LookupEntry(itr->first);
                    if (!spellProto)
                        continue;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 31658:
                {
                    *handled = true;
                    ...
                    SpellInfo const* spInfo = sSpellStore.LookupEntry(67545);
                    if (!spInfo)
                        return false;
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
        ...
        switch (auraSpellInfo->GetSpellFamilyName())
        {
            ...
            case SPELLFAMILY_HUNTER:
            {
                if (auraSpellInfo->SpellIconID == 3247)     // Piercing Shots
                {
                    ...
                    SpellInfo const* TriggerPS = sSpellStore.LookupEntry(trigger_spell_id);
                    if (!TriggerPS)
                        return false;
                ...
                switch (auraSpellInfo->Id)
                {
                    ...
                    default:
                        ...
                        if (auraSpellInfo->SpellIconID == 241)
                        {
                            ...
                            SpellInfo const* originalSpell = sSpellStore.LookupEntry(originalSpellId);
                            if (!originalSpell)
                            {
    ...
    SpellInfo const* triggerEntry = sSpellStore.LookupEntry(trigger_spell_id);
        ...
        case 53817:
        {
            ...
            if (AuraEffect const* aurEff = GetAuraEffect(70832, 0))
                if (Aura const* maelstrom = GetAura(53817))
                    if ((maelstrom->GetStackAmount() == maelstrom->GetSpellInfo()->GetStackAmount()) && roll_chance_i(aurEff->GetAmount()))
...
void Unit::SetMinion(Minion *minion, bool apply)
{
    ...
    if (apply)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            ...
            SpellInfo const* spellInfo = sSpellStore.LookupEntry(minion->GetUInt32Value(UNIT_CREATED_BY_SPELL));
    ...
    else
    {
        ...
        if (const SpellInfo* spInfo = sSpellStore.LookupEntry(minion->ToTotem()->GetSpell()))
...
uint32 Unit::SpellDamageBonus(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (spellProto)
    {
        ...
        for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
            ...
            {
                if ((*i)->GetSpellInfo()->GetEquippedItemClass() == -1)
                {
                    AddPctN(DoneTotalMod, (*i)->GetAmount());
                }
                else if (!((*i)->GetSpellInfo()->AttributesEx5 & SPELL_ATTR5_SPECIAL_ITEM_CLASS_CHECK))
                {
                    if ((*i)->GetSpellInfo()->GetEquippedItemClass() & spellProto->GetEquippedItemClass())
                        if (((*i)->GetSpellInfo()->GetEquippedItemSubClassMask() == 0) ||
                            ((*i)->GetSpellInfo()->GetEquippedItemSubClassMask() & spellProto->GetEquippedItemSubClassMask()))
                            AddPctN(DoneTotalMod, (*i)->GetAmount());
                }
            ...
            if (GetTypeId() == TYPEID_PLAYER)
            {
                ...
                for (PlayerSpellMap::const_iterator itr = playerSpells.begin(); itr != playerSpells.end(); ++itr)
                {
                    ...
                    switch (itr->first)
                    {
                        ...
                        case 49638:
                        {
                            if (SpellInfo const* proto = sSpellStore.LookupEntry(itr->first))
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask)
{
    ...
    for (AuraEffectList::const_iterator i = mDamageDone.begin(); i != mDamageDone.end(); ++i)
        if (((*i)->GetMiscValue() & schoolMask) != 0 &&
            (*i)->GetSpellInfo()->GetEquippedItemClass() == -1 &&
            // -1 == any item class (not wand then)
            (*i)->GetSpellInfo()->GetEquippedItemInventoryTypeMask() == 0)
...
int32 Unit::SpellBaseHealingBonus(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (AuraEffectList::const_iterator i = mHealingDoneOfStatPercent.begin(); i != mHealingDoneOfStatPercent.end(); ++i)
        {
            ...
            Stats usedStat = Stats((*i)->GetSpellInfo()->GetEffectMiscValue((*i)->GetEffIndex()));
...
void Unit::MeleeDamageBonus(Unit *pVictim, uint32 *pdamage, WeaponAttackType attType, SpellInfo const *spellProto)
{
    ...
    for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
        if (spellProto)
        {
                if ((*i)->GetMiscValue() & GetSpellSchoolMask(spellProto))
                {
                    ...
                    {
                        if ((*i)->GetSpellInfo()->GetEquippedItemClass() & spellProto->GetEquippedItemSubClassMask())
                            if (((*i)->GetSpellInfo()->GetEquippedItemSubClassMask() == 0) ||
                                ((*i)->GetSpellInfo()->GetEquippedItemSubClassMask() & spellProto->GetEquippedItemSubClassMask()))
                                AddPctN(DoneTotalMod, (*i)->GetAmount());
                    }
    ...
    for (AuraEffectList::const_iterator i = mDummyAuras.begin(); i != mDummyAuras.end(); ++i)
    {
        switch((*i)->GetSpellInfo()->SpellIconID)
        {
            ...
            case 1804:
            {
                if ((*i)->GetSpellInfo()->GetSpellFamilyName() != SPELLFAMILY_PALADIN)
                    continue;
...
bool Unit::IsPolymorphed() const
{
    ...
    SpellInfo const* spellInfo = sSpellStore.LookupEntry(transformId);
    if (!spellInfo)
        return false;
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
    ...
    for (SpellClickInfoMap::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        if (itr->second.IsFitToRequirements(clicker, this))
        {
            ...
            SpellInfo const* SpellInfo = sSpellStore.LookupEntry(itr->second.spellId);
            ...
            if (seatId > -1)
            {
                ...
                {
                    if (SpellInfo->GetEffectApplyAuraNameByIndex(i) == SPELL_AURA_CONTROL_VEHICLE)
                    {
                        valid = true;
                        break;
                    }
